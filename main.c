...
char uMap[5][30][30];           // undo 사용전 맵을 저장하는 배열
...
void command() { 
    ...
    while (1) {
        ...
        if (ch == 't') { // 전체 맵 순위만 출력할지 특정한 맵 순위만 출력할건지 다시 입력받아야 하므로 if문을 통해 구분함
            ...
            switch (ch) {
                case 'u':
        			if (uMap[0][0][0]=='\0'){ // uMap배열의 첫번째 배열이 null 값이면 되돌리기가 불가능하다는 메세지 출력
                        system("clear");
                        printMap();
		    		    printf("\n남은 되돌리기 횟수가 0이 되어 더 이상 되돌리기가 불가능 합니다.");
                    } else { // uMap 배열의 첫번째 배열이 null 값이 아니면 이동횟수를 1회 올리고 undo()함수 실행
			        	system("clear");
        				mvCnt++;
	        			undo();
                    }
